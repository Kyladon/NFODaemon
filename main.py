...
def save_sfv_files(sfvs, hexdig, save):
    ...
    for _, sfv in enumerate(sfvs):
        ...
        saved_paths.append({
            'sfv_data_path': sfv_data_path if save else None,
...
def save_mi_files(mis, hexdig, save):
    ...
    for _, mi in enumerate(mis):
        ...
        if save:
            ...
            saved_paths.append({
                'mi_data_path': mi_data_path if save else None,
                'miname': miname,
                'mipath': mipath,
                'mi_hexdig': mi_hexdig,
...
def override_commercial_name(name: str) -> str:
    # If 'name' is in the AUDIO_NAME_OVERRIDES dict then return the shorter name
...
def normalized_section_name(sec_name: str) -> Literal['video', 'audio', 'text', 'other']:
...
def parse_mediainfo_file(mi_data) -> dict[str, list[dict]]:
...
async def upload_nfo(request):
    ...
    try:
        ...
        threading.Thread(
            target=CleanupFiles,
            args=(
                image_path, release_info_path, nfo_data_path, filename_info_path, saved_sfv_paths, saved_mi_paths, timeout_seconds
            )).start()
...
async def serve_image(_, filename):
    ...
    if not mi_json_override or (len(video_tracks) == 0 and len(audio_tracks) == 0 and mediainfo):
...
async def download_nfo(_, filename):
    # Download NFO file
    filename_info_path = f"static/{filename}_filename.txt"
...
async def download_mi(_, mi_hexdig):
    # Determine the correct NFO hash by finding the relevant metadata file
    for root, _, files in os.walk('static'):
        for miFile in files:
            if not miFile.endswith('_mi_metadata.txt'):
                continue
...
async def download_all(_, filename):
    release_info_path = f"static/{filename}.txt"
    filename_info_path = f"static/{filename}_filename.txt"
    nfo_data_path = f"static/{filename}.nfo"
...
async def serve_themes(_, filename):
...
async def serve_svg_icons(_, filename):
...
def CleanupFiles(image_path, release_info_path, nfo_data_path, filename_info_path, saved_sfv_paths, saved_mi_paths, delay):
    ...
    if image_path:
        paths_to_remove = [
            image_path, release_info_path, nfo_data_path, filename_info_path,
            f"{os.path.splitext(image_path)[0]}_optional.txt",
            f"{os.path.splitext(image_path)[0]}_sfv_metadata.txt",
            f"{os.path.splitext(image_path)[0]}_mediainfo.json",
            f"{os.path.splitext(image_path)[0]}_mi_metadata.txt"
        ]
    ...
    for sfv in saved_sfv_paths:
        if sfv['sfv_data_path'] and os.path.exists(sfv['sfv_data_path']):
            os.remove(sfv['sfv_data_path'])
        if os.path.exists(sfv['sfv_image_path']):
            os.remove(sfv['sfv_image_path'])
 
    for mi in saved_mi_paths:
        if mi['mi_data_path'] and os.path.exists(mi['mi_data_path']):
